...
const AnalysesPage: React.FC = () => {
    const { employees, logs, isScheduleSet } = useStore();
    ...
    const statsSummary = useMemo(() => {
        if (!monthlyData) return { workedDaysCount: 0, lateDaysCount: 0, lateRate: 0 };
        const workedDays = monthlyData.filter(d => d.segments.length > 0);
        const lateDays = workedDays.filter(d => d.segments.some(s => s.type === 'late'));
        return {
            workedDaysCount: workedDays.length,
            lateDaysCount: lateDays.length,
            lateRate: workedDays.length > 0 ? (lateDays.length / workedDays.length) * 100 : 0
        };
    }, [monthlyData]);
    ...
    return (
        <div className="relative min-h-[calc(100vh-80px)]">
            <div className={`p-4 md:p-6 max-w-5xl mx-auto space-y-8 pb-24 transition-all duration-700 ${!isScheduleSet ? 'blur-md pointer-events-none select-none opacity-50 grayscale' : ''}`}>
                ...
                {selectedEmployee && (
                    <section className="bg-[#1e3a8a] rounded-[40px] shadow-2xl overflow-hidden text-white flex flex-col min-h-[600px] border border-white/5">
                        ...
                        <div className="p-8 pb-4 space-y-6">
                            ...
                            {statsSummary.lateRate > 30 && (
                                <div className="mx-6 p-4 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 animate-in slide-in-from-top duration-500">
                                    <div className="flex items-center justify-between gap-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <p className="text-sm font-bold text-white leading-tight">
                                                💡 Ce profil semble avoir beaucoup d'anomalies.
                                                <span className="block text-blue-200 text-xs font-normal mt-0.5">Est-ce le bon planning ?</span>
                                            </p>
                                        </div>
                                        <button
                                            onClick={() => navigate('/admin/employees')}
                                            className="px-4 py-2 bg-white text-blue-900 text-xs font-black rounded-lg hover:bg-cyan-50 transition-colors shadow-lg pointer-events-auto"
                                        >
                                            CHANGER POUR UN PLANNING SPÉCIFIQUE
                                        </button>
                                    </div>
                                </div>
                            )}
            ...
            {/* Ghost Dashboard Overlay */}
            {!isScheduleSet && (
                <div className="absolute inset-0 z-[100] flex items-center justify-center p-6 bg-white/20 backdrop-blur-sm animate-in fade-in duration-700">
                    <div className="max-w-md w-full bg-white rounded-[40px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] p-10 text-center border border-gray-100 flex flex-col items-center gap-6">
                        <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center text-blue-600 shadow-inner">
                            <Zap size={40} fill="currentColor" />
                        </div>
                        <div className="space-y-2">
                            <h3 className="text-2xl font-black text-gray-900 uppercase tracking-tight">Analyses verrouillées</h3>
                            <p className="text-gray-500 font-medium">Configurez votre planning global pour débloquer les statistiques de présence, retards et heures supplémentaires.</p>
                        </div>
                        <button
                            onClick={() => navigate('/admin/employees')}
                            className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black text-sm uppercase tracking-[0.1em] shadow-[0_10px_20px_rgba(37,99,235,0.3)] hover:bg-blue-700 hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-3"
                        >
                            <Zap size={18} fill="currentColor" />
                            Débloquez vos analyses
                        </button>
                    </div>
                </div>
            )}
